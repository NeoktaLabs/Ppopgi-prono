...
export async function me(request: Request, env: Env) {
  ...
  const leagues = await env.DB.prepare(`
    SELECT leagues.id, leagues.name, leagues.code, league_members.role
    FROM league_members
    JOIN leagues ON leagues.id = league_members.league_id
    WHERE league_members.user_id = ? AND league_members.removed_at IS NULL
    ORDER BY league_members.joined_at DESC
  `).bind(user.id).all();
...
export async function updateProfile(request: Request, env: Env) {
  ...
  if (!clean || clean.length < 2 || clean.length > 30) {
    return badRequest("Le pseudo doit contenir entre 2 et 30 caractères.");
  }

  await env.DB.prepare("UPDATE users SET nickname = ?, updated_at = ? WHERE id = ?")
    .bind(clean, nowIso(), user.id)
    .run();
...
export async function createLeague(request: Request, env: Env) {
  ...
  if (!clean || clean.length < 3) return badRequest("Nom de ligue invalide.");
  ...
  await env.DB.batch([
    env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, 1, ?, ?)
    `).bind(leagueId, clean, code, user.id, nowIso(), nowIso()),
    env.DB.prepare(`
      INSERT INTO league_members (id, league_id, user_id, role, joined_at)
      VALUES (?, ?, ?, 'admin', ?)
    `).bind(crypto.randomUUID(), leagueId, user.id, nowIso()),
  ]);
...
export async function joinLeague(request: Request, env: Env) {
  ...
  if (!clean) return badRequest("Code de ligue obligatoire.");
  ...
  if (!league) return badRequest("Ligue introuvable.", 404);
  if (!league.is_joinable) return badRequest("Cette ligue est fermée aux inscriptions.", 403);
...
export async function upsertPrediction(request: Request, env: Env, leagueId: string) {
  ...
  if (!matchId || homeScore === undefined || awayScore === undefined) return badRequest("Prono incomplet.");

  const membership = await env.DB.prepare(`
    SELECT id FROM league_members WHERE league_id = ? AND user_id = ? AND removed_at IS NULL
  `).bind(leagueId, user.id).first();
  if (!membership) return badRequest("Tu n’es pas membre de cette ligue.", 403);
  ...
  if (Date.now() >= new Date(match.kickoff_at).getTime()) {
    return badRequest("Le coup d’envoi est passé, le prono est verrouillé.", 409);
  }

  await env.DB.prepare(`
    INSERT INTO predictions (id, league_id, user_id, match_id, home_score, away_score, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(league_id, user_id, match_id) DO UPDATE SET
      home_score = excluded.home_score,
      away_score = excluded.away_score,
      updated_at = excluded.updated_at
  `).bind(crypto.randomUUID(), leagueId, user.id, matchId, homeScore, awayScore, nowIso(), nowIso()).run();
...
export async function matchPredictions(request: Request, env: Env, leagueId: string, matchId: string) {
  ...
  const rows = hasStarted
    ? await env.DB.prepare(query).bind(leagueId, matchId).all()
    : await env.DB.prepare(query).bind(leagueId, matchId, user.id).all();
...
export async function removeLeagueMember(request: Request, env: Env, leagueId: string, userId: string) {
  ...
  if (!league) return badRequest("Ligue introuvable.", 404);
  if (league.admin_user_id !== user.id) return badRequest("Réservé à l’admin de la ligue.", 403);
  if (userId === user.id) return badRequest("L’admin ne peut pas se supprimer lui-même.", 409);

  await env.DB.prepare(`
    UPDATE league_members SET removed_at = ?, removed_by_user_id = ?
...
export async function recalculateMatch(env: Env, matchId: string) {
  ...
  `).bind(matchId).all<{ id: string; home_score: number; away_score: number }>();
  ...
  for (const prediction of predictions.results ?? []) {
    ...
    await env.DB.prepare(`
      UPDATE predictions SET points = ?, is_exact = ?, is_correct_result = ?, updated_at = ? WHERE id = ?
    `).bind(score.points, score.isExact ? 1 : 0, score.isCorrectResult ? 1 : 0, nowIso(), prediction.id).run();
  }
}
